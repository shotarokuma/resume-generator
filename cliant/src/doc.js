...
export const createDocx = (user) => {
  axios.get(`/api/v1/educations/${user._id}`)
    .then((res) => {
      const educationDocx = res.data.education.map((edu) => {
        if (edu.note) {
      ...
      axios.get(`/api/v1/experiences/${user._id}`)
        .then((res) => {
          const experienceDocx = res.data.experience.map((ex) => {
            if (ex.note) {
              return new Paragraph({
                spacing: {
                  before: 100,
